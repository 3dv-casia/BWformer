...
class HeatEdge(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_feature_levels, backbone_strides, backbone_num_channels, ):
        ...
        self.edge_input_fc2 = nn.Linear(384, hidden_dim)
    ...
    def forward(self, image_feats, feat_mask, corner_outputs, edge_coords, edge_masks, gt_values, corner_nums,
        ...
        edge_feats = list()
        div_term = torch.exp( torch.arange(0, 192 // 3, 2, dtype=torch.float, device=edge_coords.device) *  (-math.log(10000.0) / (192 // 3)))
        div_term = div_term.view( 1,1, -1) 


        for b_i in range(bs):
            
            x_position, y_position, z_position = edge_coords[b_i, :, :, 0:1], edge_coords[b_i, :, :,1:2], edge_coords[b_i, :, :,2:3]
            ...
            edge_feats.append(position_code)
        edge_feats = torch.stack(edge_feats, dim=0)
        edge_feats = edge_feats.view(bs, num_edges, -1)


        edge_inputs = self.edge_input_fc2(edge_feats.view(bs * num_edges, -1))
...
class EdgeTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, reference_points, labels, key_padding_mask, corner_nums,
                max_candidates, do_inference=False, edge_coords=None, feat_mask=None):
        src_flatten = []
        mask_flatten = []
        ...
        filtered_hs, filtered_mask, filtered_query, filtered_rp, filtered_labels, selected_ids, filterd_ec = self.candidate_filtering(
            logits_per_edge,
            hs_per_edge, query_embed, reference_points,
            labels,
            key_padding_mask, corner_nums, max_candidates, edge_coords)
        ...
        edge_points = filterd_ec[..., :2]
        num_sample = 5
        t = torch.linspace(0, 1, steps=num_sample).to(filterd_ec.device)
        t = t.view(1, 1, num_sample, 1)
        start_points = edge_points[..., 0, :]  # [batch_size, num, 2]
        end_points = edge_points[..., 1, :]  # [batch_size, num, 2]
        interpolated_points = (1 - t) * start_points.unsqueeze(2) + t * end_points.unsqueeze(2)  # [batch_size, num, n, 2]
        ...
        hybrid_prim_hs_sample = hybrid_prim_hs.unsqueeze(2).repeat(1, 1, num_sample, 1).view(bs, hybrid_prim_hs.shape[1]*num_sample, hybrid_prim_hs.shape[-1])
        rel_prim_hs_sample = rel_prim_hs.unsqueeze(2).repeat(1, 1, num_sample, 1).view(bs, rel_prim_hs.shape[1]*num_sample, rel_prim_hs.shape[-1])
        filtered_query_sample = filtered_query.unsqueeze(2).repeat(1, 1, num_sample, 1).view(bs, filtered_query.shape[1]*num_sample, filtered_query.shape[-1])
        filtered_mask_sample = filtered_mask.unsqueeze(2).repeat(1, 1, num_sample).view(bs, filtered_mask.shape[1]*num_sample)
        interpolated_points = interpolated_points.view(bs, interpolated_points.shape[1]*num_sample, interpolated_points.shape[-1])
        hs, inter_references = self.relational_decoder(hybrid_prim_hs_sample, interpolated_points, memory,
                                                        spatial_shapes, level_start_index, valid_ratios, filtered_query_sample,
                                                        mask_flatten,
                                                        key_padding_mask=filtered_mask_sample, get_image_feat=True)



        hs_rel, _ = self.relational_decoder(rel_prim_hs_sample, interpolated_points,  memory,
                                                spatial_shapes, level_start_index, valid_ratios, filtered_query_sample,
                                                mask_flatten,
                                                key_padding_mask=filtered_mask_sample, get_image_feat=False)
        hs, _  = hs.view(hs.shape[0], filterd_ec.shape[1], num_sample, -1).max(axis=2)
        hs_rel, _ = hs_rel.view(hs_rel.shape[0], filterd_ec.shape[1], num_sample, -1).max(axis=2)
    ...
    @staticmethod
    def candidate_filtering(logits, hs, query, rp, labels, key_padding_mask, corner_nums, max_candidates, edge_coords):
        B, L, _ = hs.shape
        ...
        for b_i in range(B):
            ...
            filtered_ec.append(edge_coords[b_i][ids])
        ...
        filtered_ec = torch.stack(filtered_ec, dim=0)


        return filtered_hs, filtered_mask, filtered_query, filtered_rp, filtered_labels, selected_ids, filtered_ec
    @staticmethod
